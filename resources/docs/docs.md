...
The `home-routes` handler in the `realworld-clj.routes.home` namespace
...
The `home-page` function will in turn call the `realworld-clj.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `realworld-clj.handler` namespace:
...
Request middleware functions are located under the `realworld-clj.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `realworld-clj.dev-middleware` namespace found in
...
* Let `mount` know to start the Crux node by `require`-ing `realworld-clj.db.core` in some other namespace.
